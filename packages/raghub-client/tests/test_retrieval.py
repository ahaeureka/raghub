...
class TestRetrievalService:
    ...
    @pytest.mark.asyncio
    async def test_retrieval_basic(self, shared_index: BaseRAGTest):
        """测试基本检索功能"""
        query = TestConfig.TEST_QUERIES[0]
        request = shared_index.create_test_retrieval_request(query, top_k=3)

        response = await shared_index.client.rag_service_retrieval(request)

        shared_index.assert_retrieval_response(response, expected_min_records=1)
    ...
    @pytest.mark.asyncio
    async def test_retrieval_with_different_top_k(self, shared_index: BaseRAGTest):
        """测试不同 top_k 值的检索"""
        query = TestConfig.TEST_QUERIES[1]

        # 测试 top_k=1
        request_1 = shared_index.create_test_retrieval_request(query, top_k=1)
        response_1 = await shared_index.client.rag_service_retrieval(request_1)
        assert len(response_1.records) <= 1

        # 测试 top_k=3
        request_3 = shared_index.create_test_retrieval_request(query, top_k=3)
        response_3 = await shared_index.client.rag_service_retrieval(request_3)
    ...
    @pytest.mark.asyncio
    async def test_retrieval_with_metadata_filter(self, shared_index: BaseRAGTest):
        """测试带元数据过滤的检索"""
        query = "人工智能"
        ...
        request = shared_index.create_test_retrieval_request(query, top_k=5)
    ...
    @pytest.mark.asyncio
    async def test_retrieval_empty_query(self, shared_index: BaseRAGTest):
        """测试空查询"""
        request = shared_index.create_test_retrieval_request("", top_k=3)
    ...
    @pytest.mark.asyncio
    async def test_retrieval_nonexistent_knowledge_id(self, shared_index: BaseRAGTest):
        """测试不存在的知识库ID"""
        request = shared_index.create_test_retrieval_request(TestConfig.TEST_QUERIES[0], top_k=3)
    ...
    @pytest.mark.asyncio
    async def test_retrieval_score_ordering(self, shared_index: BaseRAGTest):
        """测试检索结果按相关性得分排序"""
    ...
    @pytest.mark.asyncio
    async def test_retrieval_response_fields(self, shared_index: BaseRAGTest):
        """测试检索响应字段完整性"""
        query = TestConfig.TEST_QUERIES[0]
        request = shared_index.create_test_retrieval_request(query, top_k=1)
